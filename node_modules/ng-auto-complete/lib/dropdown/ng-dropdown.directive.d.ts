import { ElementRef, EventEmitter, OnChanges, OnDestroy, OnInit, SimpleChanges, ChangeDetectorRef } from '@angular/core';
export declare class NgDropdownDirective implements OnChanges, OnInit, OnDestroy {
    _eref: ElementRef;
    list: any[];
    active: any;
    parentDetector: ChangeDetectorRef;
    input: HTMLElement;
    element: Element;
    key: string;
    completion: boolean;
    hover: EventEmitter<any>;
    selected: EventEmitter<any>;
    closed: EventEmitter<any>;
    _open: boolean;
    _list: {
        active: boolean;
        [value: string]: any;
    }[];
    _class: string;
    constructor(_eref: ElementRef);
    /**
     *
     */
    ngOnInit(): void;
    /**
     *
     */
    ngOnChanges(changes: SimpleChanges): void;
    /**
     *
     */
    keyDown(event: KeyboardEvent): void;
    /**
     *
     */
    OnMouseOver(event: MouseEvent): void;
    /**
     *
     */
    EmitSelected(): void;
    /**
     *
     */
    DropdownFocusAreaDown(): void;
    /**
     *
     */
    DropdownFocusAreaUp(): void;
    /**
     *
     */
    readonly opened: boolean;
    /**
     *
     */
    Close(event: any, force?: boolean): void;
    /**
     *
     */
    private inputKeydown;
    private inputKeydownBind;
    /**
     *
     */
    private documentKeydown;
    private documentKeydownBind;
    /**
     *
     */
    private mouseoverListener;
    private mouseoverListenerBind;
    /**
     *
     */
    RegisterListeners(): void;
    /**
     *
     */
    RemoveListeners(): void;
    /**
     *
     */
    Open(): void;
    /**
     *
     */
    RefExists(): boolean;
    /**
     *
     */
    FindActive(): number;
    /**
     *
     */
    SetActive(index: number): void;
    /**
     *
     */
    GetElement(index: number): any;
    /**
     *
     */
    ClearActive(): void;
    /**
     *
     */
    PrepareList(): void;
    /**
     *
     */
    ActiveItem(item: any): boolean;
    /**
     *
     */
    DetermineActiveClass(): void;
    /**
     *
     */
    PrepareChildrenList(): void;
    /**
     *
     */
    DeReference(object: {
        active: boolean;
        [value: string]: any;
    }): any;
    /**
     *
     */
    ngOnDestroy(): void;
}
