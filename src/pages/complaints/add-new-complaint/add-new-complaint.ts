...
image:any='';
  ...
  fileChange(img)
 {
  ...
  genrateCompliant(){
    ...
    // if((!this.complaint_data.image.length && !this.videoId)) {
    ...
    this.service.post_rqst( {'complaint':this.complaint_data },'app_karigar/addComplaint').subscribe(result =>
      {
        ...
        this.showSuccess("Complaint or installation is Pending!");
        }
        else if(this.complaint_data.type=='service'){
      
        this.showSuccess("Complaint Added Successfully!");
        }
        else{
          this.showSuccess("Installation Request added successfully")
        }
